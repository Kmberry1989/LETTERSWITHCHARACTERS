...
import { getDocument, listDocuments, setDocument, toDocumentStoreError } from '@/lib/server/document-store';
...
export async function GET() {
  try {
    ...
    const normalized = toDocumentStoreError(error, 'Could not load direct threads.');
    const status = 'status' in normalized && typeof normalized.status === 'number' ? normalized.status : 500;
    return NextResponse.json({ error: normalized.message || 'Could not load direct threads.' }, { status });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const normalized = toDocumentStoreError(error, 'Could not create direct thread.');
    const status = 'status' in normalized && typeof normalized.status === 'number' ? normalized.status : 500;
    return NextResponse.json({ error: normalized.message || 'Could not create direct thread.' }, { status });
  }
}
